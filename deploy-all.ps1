...
# Clean up any problematic stacks or change sets
Write-Host "      Cleaning up any stale resources..." -ForegroundColor Gray
...
if ($stackStatus -match "REVIEW_IN_PROGRESS|ROLLBACK_COMPLETE") {
    Write-Host "      CDKToolkit stack is in $stackStatus state, deleting..." -ForegroundColor Yellow
...
} elseif ($stackStatus -match "CREATE_COMPLETE|UPDATE_COMPLETE") {
    Write-Host "      CDK already bootstrapped" -ForegroundColor Green
}

# Always try to bootstrap (will be no-op if already exists and up-to-date)
Push-Location cdk
...
if ($bootstrapExitCode -ne 0) {
    ...
    if ($bootstrapOutput -match "EarlyValidation|ResourceExistenceCheck") {
        ...
        Write-Host "      [ERROR] AWS Early Validation failure detected" -ForegroundColor Red
        Write-Host "" -ForegroundColor Yellow
        Write-Host "      This is a known AWS CloudFormation issue with change sets." -ForegroundColor Yellow
        Write-Host "      Workaround: Delete the failed change set and retry" -ForegroundColor Yellow
        Write-Host "" -ForegroundColor Gray
        Write-Host "      Running automated fix..." -ForegroundColor Cyan
        
        # Delete all change sets for this stack
        ...
        # Retry bootstrap
        Write-Host "      Retrying bootstrap..." -ForegroundColor Cyan
        Push-Location cdk
        $timestamp = Get-Date -Format "yyyyMMddHHmmss"
        npx cdk bootstrap --output "cdk.out.$timestamp" --no-cli-pager --no-color
        $retryExitCode = $LASTEXITCODE
        Pop-Location
        
        if ($retryExitCode -ne 0) {
            Write-Host "" -ForegroundColor Red
            Write-Host "      Bootstrap still failing. Manual intervention required:" -ForegroundColor Red
            Write-Host "      1. Run: aws cloudformation delete-stack --stack-name CDKToolkit" -ForegroundColor Cyan
            Write-Host "      2. Wait for deletion to complete" -ForegroundColor Cyan
...
Push-Location cdk
$timestamp = Get-Date -Format "yyyyMMddHHmmss"
npx cdk deploy AgentCoreRuntime --output "cdk.out.$timestamp" --no-cli-pager --require-approval never 2>&1 | Tee-Object -Variable cdkOutput | Out-Null
...
if ($deployExitCode -ne 0) {
    ...
    if ($cdkOutput -match "Unrecognized resource types.*BedrockAgentCore") {
        ...
        Write-Host "`n[ERROR] DEPLOYMENT FAILED: AgentCore is not available in region '$currentRegion'" -ForegroundColor Red
        Write-Host ""
